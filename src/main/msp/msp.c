    ...
    case MSP_SET_VTX_CONFIG:
        {
            ...
            if (newFrequency <= VTXCOMMON_MSP_BANDCHAN_CHKVAL) {  // Value is band and channel
                const uint8_t newBand = newFrequency / 8;
                const uint8_t newChannel = newFrequency % 8;
            ...
            if (sbufBytesRemaining(src) >= 5) {
                ...
                // We expect that the detailed band/channel and power level messages will
                // follow to repopulate the tables, so clear them in anticipation.
    ...
    case MSP_SET_VTXTABLE_BAND:
        {
            ...
            const char bandLetter = sbufReadU8(src);
